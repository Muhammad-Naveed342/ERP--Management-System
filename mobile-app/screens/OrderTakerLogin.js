...
const EXPECTED_ROLE = "order_taker";

export default function OrderTakerLogin({ onLoginSuccess, onBack }) {
    ...
    try {
      ...
      if (online) {
        ...
        if (data.user.role !== EXPECTED_ROLE) {
          Alert.alert(
            "Wrong portal",
            "This account is not an order taker. Use the Sales portal instead."
          );
          return;
        }
      ...
      >
        <View style={styles.header}>
          <View style={[styles.iconCircle, { backgroundColor: "#EEF2FF" }]}>
            <Ionicons name="cart" size={50} color="#4F46E5" />
          </View>
          <Text style={styles.title}>Order Taker Login</Text>
...
const styles = StyleSheet.create({
  ...
  backText: { fontSize: 16, color: "#4F46E5", fontWeight: "700", marginLeft: 8 },
});
