    ...
    <Modal visible={visible} animationType="slide" transparent>
      <View style={pm.overlay}>
        <View style={pm.sheet}>
          ...
          {/* Products List */}
          {loadingItems ? (
            <ActivityIndicator style={{ margin: 24 }} size="large" color="#059669" />
          ) : (
            <FlatList
              data={itemsList}
...
export default function SalesManScreen({ user, onBack }) {
  ...
  const refreshList = useCallback(async () => {
    setRecords(await listLocalSales(user.id));
  }, [user.id]);
  ...
  // Salesmen resolve the wholesale price tier
  const unit = item
    ? (unitType === "carton"
      ? Number(item.wholesale_price_carton ?? ((item.wholesale_price ?? item.price) * (item.pieces_per_carton || 12)))
      : Number(item.wholesale_price ?? item.price))
    : 0;
  ...
  const submit = async () => {
    if (!shop || !item) {
      Alert.alert("Select shop and item", "Choose from the lists.");
      return;
    }
    if (qtyNum <= 0) {
    ...
    try {
      ...
      await refreshList();
      await runSync();
      Alert.alert("Saved", "Sale stored on device. Uploads when online.");
    } catch (e) {
      Alert.alert("Error", e.message || String(e));
    }
  };

  const handleDeleteSale = (localId) => {
    Alert.alert("Delete Sale", "Are you sure you want to delete this locally saved sale?", [
    ...
    <View style={styles.card}>
      <View style={styles.cardTop}>
        ...
        <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
          ...
          <TouchableOpacity onPress={() => handleDeleteSale(row.local_id)}>
      ...
      <View style={styles.rowStats}>
        ...
        <Text style={styles.muted}>Income: Rs. {Number(row.income_received).toFixed(2)}</Text>
        <Text style={styles.muted}>Loan: Rs. {Number(row.loan).toFixed(2)}</Text>
      </View>
  ...
  return (
    <View style={styles.container}>
      <View style={styles.topBar}>
        ...
        <Text style={styles.topTitle}>Sales</Text>
        <TouchableOpacity onPress={runSync}>
          <Ionicons
            name="cloud-upload-outline"
            size={24}
            color={net ? "#059669" : "#9CA3AF"}
          />
        </TouchableOpacity>
      </View>
      {!user.access_token ? (
        <Text style={styles.banner}>
          Offline session: connect online periodically to upload pending sales.
        </Text>
      ) : null}
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <View style={styles.form}>
            <Text style={styles.hint}>
              {net ? "Online — pending sales will try to upload." : "Offline — saved locally."}
            </Text>
            ...
            <Text style={styles.label}>Income received</Text>
            <TextInput
              style={styles.input}
        ...
        ListEmptyComponent={
          loading ? (
            <ActivityIndicator style={{ marginTop: 24 }} size="large" color="#059669" />
          ) : (
...
const styles = StyleSheet.create({
  ...
  totalLabel: { fontSize: 15, fontWeight: "600", color: "#047857" },
  totalBig: { fontSize: 22, fontWeight: "900", color: "#065F46" },
  primary: {
    marginTop: 18,
  ...
  total: { marginTop: 8, fontSize: 14, fontWeight: "800", color: "#111827" },
